...
type Props = {
  routeId: "/maintenance4" | "/maintenance5" | "/maintenance6";
  multiplier: number;
};

export function MaintenanceSinglePresetPage({ routeId, multiplier }: Props) {
  const { name: nameParam, email: emailParam } = useSearch({ from: routeId }) as {
    name?: string;
    email?: string;
  };
